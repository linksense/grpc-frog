...
class ProtoHelper:
    def __init__(self, servicer_name=None, save_dir=None, proto_location=None):
        ...
        self._servicer_list = (
            (frog.servicer_map[servicer_name],)
            if servicer_name
            else frog.servicer_map.values()
        )
    ...
    def generate_code(self):
        for servicer in self._servicer_list:
            ...
            pb2_file = os.path.join(
                servicer.proto_dir, servicer.name + "_pb2.py"
            )  # pb2 决定对路径
            pb2_grpc_file = os.path.join(
                servicer.proto_dir, servicer.name + "_pb2_grpc.py"
            )  # pb2 grpc 绝对路径
    ...
    def generate_proto_file(self, servicer, proto_file, pb2_file, pb2_grpc_file):
        ...
        _proto_str = "service {name} {{\n  {method_str}\n}}".format(
            name=servicer.name, method_str=method_str
        )
        messages = "\n\n".join(
            [
    ...
    def _generate_pb2_file(self, servicer, proto_file, pb2_file, pb2_grpc_file):
        ...
        if not os.path.exists(google_dir):
            ...
            frog_google_dir = os.path.join(
                os.path.dirname(grpc_frog.__file__), "proto", "google"
            )
        ...
        file_data = file_data.replace(
            ...
            "\nfrom {} import {}".format(self._proto_location, servicer.name),
        )
    ...
    def get_message_body(servicer):
        ...
        ret = [
            define_text.format(method.name, method.request_name, method.response_name)
